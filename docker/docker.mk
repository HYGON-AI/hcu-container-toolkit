...
X86_64_TARGETS := centos7 centos8 rocky8 rocky9 kylin10
...
--%: BUILDIMAGE = hcu/$(LIB_NAME)/$(OS)$(VERSION)-$(ARCH)
...
--centos8%: DOCKERFILE = $(CURDIR)/docker/Dockerfile.rpm-yum

# private rocky target
--rocky%: OS := rocky
--rocky%: DOCKERFILE = $(CURDIR)/docker/Dockerfile.rocky
--rocky8%: BASEIMAGE = rockylinux:8
--rocky9%: BASEIMAGE = rockylinux:9

# private kylin targe
--kylin%: OS := kylin
--kylin%: DOCKERFILE = $(CURDIR)/docker/Dockerfile.kylin
...
docker-build-%:
	...
	docker pull --platform=linux/$(ARCH) docker.1ms.run/$(BASEIMAGE)
	DOCKER_BUILDKIT=1 \
	$(DOCKER) build \
	    --platform=linux/$(ARCH) \
	    --progress=plain \
...
docker-clean:
	IMAGES=$$(docker images "hcu/$(LIB_NAME)/*" --format="{{.ID}}"); \
	if [ "$${IMAGES}" != "" ]; then \
	    docker rmi -f $${IMAGES}; \
	fi
